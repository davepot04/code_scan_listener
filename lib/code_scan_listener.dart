...
class _CodeScanListenerState extends State<CodeScanListener> {
  ...
  bool _keyBoardCallback(KeyEvent keyEvent) {
    switch ((keyEvent, widget.useKeyDownEvent)) {
      ...
      case (final KeyDownEvent event, true):
        _controller.sink.add(event.character);
        String checker = event.character.toString().toUpperCase();
        if(event.character == checker){
          isUpperCase = true;
        }else{
          isUpperCase = false;
        }
  ...
  void onKeyEvent(String char) {
    ...
    if (char == _lineFeed) {
      ...
      if (isUpperCase){
        _scannedChars.add(char.toUpperCase());
      }else{
        _scannedChars.add(char.toLowerCase());
      }
    }
  }
